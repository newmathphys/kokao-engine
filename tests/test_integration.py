...
class TestIntegration:
    ...
    def test_core_with_counterfactual(self):
        core = KokaoCoreV9(n_features=5)
    ...
    def test_learnable_with_counterfactual(self):
        core = KokaoCoreV9(n_features=5)
        lf = KokaoCoreWithLearnableForget(core)
        cf = CounterfactualKokao(core)
        x = torch.randn(5, device="cpu")
    ...
    def test_engine_with_kg(self):
        ...
        kg.update_from_stream(x_plus, S=result["S"])

    def test_multiple_cores(self):
        cores = [KokaoCoreV9(n_features=5) for _ in range(3)]
        for core in cores:
            x = torch.randn(5, device="cpu")
            core.train(x, target=100.0)
        # Нормализация делает w.abs().sum() = 100, а не w.sum()
    ...
    def test_counterfactual_with_decoder(self):
        core = KokaoCoreV9(n_features=5)
        cf = CounterfactualKokao(core)
        decoder = KokaoDecoder(core)
        x = torch.randn(5, device="cpu")
